# -*- coding: utf-8 -*-
"""
Created on Fri Jul 20 15:36:28 2018
...
'''
def cal_PSNR(true_img, test_img):
    #assume RGB image / Y image
    true_data = np.array(true_img)
    test_data = np.array(test_img)
    
    return ski.compare_psnr(true_data, test_data)
 '''   
...
def test():
    ...
    #%% single img of one video
    '''
    #test video && img info 
    rain_path = './data/testing_data/test_data_syn/Rain/s8_Rain/'      #the test video path
    num_img=len(glob.glob(rain_path+'/*.jpg'))
    img_index = num_img-2                                                        #the index of the img to derain
    test_WIDTH = 352
    test_HEIGHT = 288
    out_path = './gxd_result/'              #the dir of the derained img to store
    ...
    X0_derained_Y=net.train_network(is_train, X0_test, Xm1_test, Xm2_test, Xp1_test, Xp2_test, 1)
    
    #Load model
    saver = tf.train.Saver()
    saver.restore(sess, './model_test/v1/model.ckpt-49000') 
    
    ##Import data 
    x1_test, x2_test, x3_test, x4_test, x5_test, x3_YCbCr = read_data(rain_path, img_index-1)
    test_feed_dict = {
            is_train: False, 
            X0_test: x3_test, 
            Xm1_test: x2_test, 
    ...
    ##save as jpg
    out_name=out_path + str("%05d" % (img_index)) + '.jpg'
    derained_img.save(out_name)
    
    '''
    
    
    #%% all imgs of many video
    ...
    if only_DR_subnet_flag:
        out_path = './EEVRSR_result/net_derain_only/' 
    else:
        out_path = './EEVRSR_result/net_all/' 
    ...
    if only_DR_subnet_flag:
        X0_derained_Y=net.train_or_test_network_drain_only(is_train, X0_test, Xm1_test, Xm2_test, Xp1_test, Xp2_test, 1) # test only the de-raining sub-net
    else:
        X0_derained_Y=net.test_network(is_train, X0_test, Xm1_test, Xm2_test, Xp1_test, Xp2_test, 1)  # test EEVRSR
